...
from app.utils.streaming_utils import should_stream_node_output, generate_content_with_stream_fallback
...
def summary_agent(state: AgentState) -> dict:
    ...
    step = current_step_map.get("summary_agent") if isinstance(current_step_map, dict) else {}
    task_description = (step or {}).get("task_description") or "汇总已有结果并生成最终结论"

    summary_input = _build_summary_input(state)
    has_data = bool(summary_input["order_context"] or summary_input["excel_context"] or summary_input["results"])
    ...
    should_stream = should_stream_node_output(state, "summary_agent")
    ...
    try:
        ...
        content, stream_chunks = generate_content_with_stream_fallback(
            llm=llm,
            messages=messages,
            enable_stream=should_stream,
        )
    except Exception:
    ...
    results = dict(state.get("results") or {})
    summary_result: dict[str, Any] = {"content": content, "is_end": should_stream}
    if should_stream and stream_chunks:
        summary_result["stream_chunks"] = stream_chunks
