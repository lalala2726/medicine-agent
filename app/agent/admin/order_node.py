...
from app.utils.streaming_utils import generate_content_with_stream_fallback, should_stream_node_output
...
def order_agent(state: AgentState) -> dict:
    ...
    step_is_end = should_stream_node_output(state, "order_agent")

    llm = create_chat_model(
        model="qwen3-max"
    )
    messages = SystemMessagePromptTemplate.from_template(template=system_prompt).format_messages(instruction=instruction)
    content, stream_chunks = generate_content_with_stream_fallback(
        llm=llm,
        messages=messages,
        enable_stream=step_is_end,
    )
