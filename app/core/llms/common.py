...
def resolve_provider_extra_body(
    ...
    if resolved_provider is LlmProvider.ALIYUN:
        ...
        thinking_payload: dict[str, Any] = (
            dict(raw_thinking_payload)
            if isinstance(raw_thinking_payload, Mapping)
            else {}
        )
        thinking_payload["type"] = "enabled" if think else "disabled"
        resolved_extra_body["thinking"] = thinking_payload
