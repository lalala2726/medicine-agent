...
class KnowledgeChunkRebuildCommandMessage(BaseModel):
    ...
    Attributes:
        message_type: 固定消息类型，值为 ``knowledge_chunk_rebuild_command``。
        task_uuid: 业务侧生成的任务唯一标识。
        knowledge_name: Milvus collection 名称。
        document_id: 业务文档 ID。
        vector_id: Milvus 向量主键 ID。
        version: 当前切片编辑版本号，用于 latest-only 判定。
        content: 修改后的切片文本内容。
        embedding_model: 本次任务指定的向量模型名称。
...
class KnowledgeChunkRebuildResultMessage(BaseModel):
    ...
    Attributes:
        message_type: 固定消息类型，值为 ``knowledge_chunk_rebuild_result``。
        task_uuid: 对应 command 的任务 ID。
        version: 对应 command 的切片版本号。
        stage: 当前任务阶段。
    ...
    def build(
        ...
        Returns:
            KnowledgeChunkRebuildResultMessage: 结构完整的结果消息对象。
        """
