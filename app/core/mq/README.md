...
├── connection.py                               # RabbitMQ URL / connection / channel 共享入口
├── config/
│   ├── common.py                               # prefetch / consumer 开关等代码常量
│   ├── topology.py                             # exchange / queue / routing key 固定约定
│   └── document/
...
- 仅 `RABBITMQ_URL` 通过环境变量提供连接地址。
- exchange / queue / routing key、prefetch 数量、latest-version key 前缀、应用内 consumer 开关均为代码常量。
- `config/document/*.py` 只负责返回固定业务约定配置；RabbitMQ 连接与 channel 打开统一走 `connection.py`。
