...
class BasePromptMiddleware(AgentMiddleware):
    ...
    def __init__(
        ...
        self._base_prompt_text = normalize_text(load_prompt(base_prompt_file))

    def _build_base_section(self) -> str:
        """构建基础提示词段落文本。"""

        return self._base_prompt_text
    ...
    def _inject_base_prompt(self, request: ModelRequest) -> ModelRequest:
        ...
        if not current_text:
            ...
            skills_prefix, skills_suffix = split_template(SKILLS_SYSTEM_PROMPT)
