...
def search_knowledge_hits(
        *,
        ...
        ranking_enabled: bool,
        build_milvus_client: Callable[..., MilvusClient],
        build_embedding_client: Callable[..., Any],
    ...
    Args:
        ...
        build_milvus_client: Milvus 客户端构造函数。
        build_embedding_client: 向量模型客户端构造函数。

    Returns:
        聚合去重并按分数降序裁剪后的候选命中列表。
    """

    client = build_milvus_client(runtime_config=runtime_config)
    embedding_client = build_embedding_client(runtime_config=runtime_config)
