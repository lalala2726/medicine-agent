...
`after_sale_sub_agent` 负责商城售后申请全流程监控。它是典型的“中继节点”，连接订单、用户与商品三大核心域。
...
1. **Step 1 (After-sale)**: `after_sale_sub_agent("查询售后单 5001 的深度详情，提取 orderId, userId, productId")`。
2. **Step 2 (Parallel)**:
    - 调用 `order_sub_agent` 查物流。
    - 调用 `user_sub_agent` 查信用与钱包。
    - 调用 `product_sub_agent` 查规格。
...
|:---------------|:------------|:--------------------------|
| “这笔订单什么时候发的货？” | `orderId`   | `order_sub_agent` (查发货记录) |
| “退款退到用户钱包了吗？”  | `userId`    | `user_sub_agent` (查流水)    |
| “退货的商品是什么规格？”  | `productId` | `product_sub_agent`       |
