...
def test_product_instruction_uses_node_goal_without_chat_history_in_supervisor_loop():
    payload = json.loads(
        product_module._build_instruction(
            _build_state("supervisor_loop", node_goal="仅核验商品2001库存并返回上下架建议")
        )
    )
    assert payload["execution_mode"] == "supervisor_loop"
    assert payload["node_goal"] == "仅核验商品2001库存并返回上下架建议"
