...
def test_model_switch_returns_high_model_and_think_enabled() -> None:
    """测试目的：验证 high 难度的模型路由；预期结果：返回 qwen3-plus 且 think=True。"""

    model_name, think = model_switch({"routing": {"task_difficulty": "high"}})

    assert model_name == "qwen3-plus"
    assert think is True


def test_model_switch_falls_back_to_default_when_difficulty_missing_or_unknown() -> None:
    ...
    model_name_legacy_simple, think_legacy_simple = model_switch({"routing": {"task_difficulty": "simple"}})
    model_name_legacy_complex, think_legacy_complex = model_switch({"routing": {"task_difficulty": "complex"}})
    ...
    assert model_name_legacy_simple == "qwen-max"
    assert think_legacy_simple is False
    assert model_name_legacy_complex == "qwen-max"
    assert think_legacy_complex is False
