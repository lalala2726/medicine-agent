...
def test_handle_incoming_message_drops_stale_command(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "app.core.mq.consumers.document.import_consumer.is_stale",
        lambda **_kwargs: True,
    )
    ...
    monkeypatch.setattr(
        "app.core.mq.consumers.document.import_consumer.publish_import_result",
        _fake_publish,
    )
...
def test_handle_incoming_message_emits_stage_events_on_success(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "app.core.mq.consumers.document.import_consumer.import_single_file",
